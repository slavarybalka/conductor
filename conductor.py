...
for elem2 in r.json():
	#print elem2
	if ("/programs/" in elem2['targetUrl'] or elem2['targetUrl'] == 'http://academicpartnerships.uta.edu') and elem2['ranks']['TRUE_RANK'] < 20 and elem2['targetUrl'] != "http://academicpartnerships.uta.edu/programs/":
...
for key, value in combined_data.iteritems():
	...
	time.sleep(3)										
	# getting related searches for each keyword
	...
	result = s.text.split('\r') 						
	# if nothing is found then just skip it
	if result == [u'ERROR 50 :: NOTHING FOUND']:		
		pass
	# if related keywords are found, add them to the dictionary	
	else:
		# print the source keyword
		print "\nSource keyword:", value[2]
		print "\nRelated keywords:"
		
		#if any related keyword is found then retrieve the contents of the related URL

		pr = requests.get(value[1])
		# iterate over the list of returned related keywords
		for item in result[1:]:	
		# if some related keywords are returned then 						
